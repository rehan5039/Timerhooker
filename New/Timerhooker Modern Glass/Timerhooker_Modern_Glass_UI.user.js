...
(function() {
  ...
  const STORAGE_KEY_DEFAULT = 'tm_default_speed';
  ...
  let defaultFast = (() => { try { const v = parseFloat(localStorage.getItem(STORAGE_KEY_DEFAULT)); return isFinite(v) && v > 0 ? v : SPEED_FAST; } catch { return SPEED_FAST; } })();
  let uiPos = (() => {
    try {
  ...
  function createUI() {
    ...
    ctrl.id = 'tm-ctrl';
    ctrl.style.cssText = `position:fixed;z-index:${UI_ZINDEX};min-width:190px;` +
      `padding:10px 10px;border-radius:12px;backdrop-filter:blur(10px);` +
      `webkit-backdrop-filter:blur(10px);background:rgba(20,28,40,0.55);` +
      `border:1px solid rgba(120,180,210,0.22);box-shadow:0 10px 28px rgba(0,0,0,0.22);` +
      `color:#e8f6ff;font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;font-size:13px;display:none;`;
    const bClose = document.createElement('button');
    bClose.textContent = '×';
    bClose.style.cssText = `position:absolute;top:6px;right:6px;width:22px;height:22px;`+
      `border-radius:11px;border:1px solid rgba(140,200,230,.25);background:rgba(255,255,255,.08);`+
      `color:#e7f7ff;cursor:pointer;line-height:20px;text-align:center;`+
      `box-shadow:0 2px 10px rgba(0,0,0,.16);transition:transform .08s,background .18s;`;
    bClose.onmouseenter = () => bClose.style.background = 'rgba(255,255,255,.14)';
    bClose.onmouseleave = () => bClose.style.background = 'rgba(255,255,255,.08)';
    bClose.onmousedown = () => bClose.style.transform = 'scale(.95)';
    bClose.onmouseup = () => bClose.style.transform = '';
    bClose.onclick = (e) => { e.preventDefault(); hideCtrl(); };
    const row = document.createElement('div');
    row.style.display = 'grid';
    row.style.gridTemplateColumns = 'repeat(6, minmax(0,1fr))';
    row.style.gap = '8px';
    function mkBtn(txt){ const b=document.createElement('button'); b.textContent=txt; b.style.cssText=`padding:7px 8px;border-radius:9px;border:1px solid rgba(140,200,230,.25);`+
      `background:rgba(255,255,255,.08);color:#e7f7ff;cursor:pointer;outline:none;`+
      `box-shadow:0 2px 10px rgba(0,0,0,.16);transition:transform .08s,background .18s`; b.onmouseenter=()=>b.style.background='rgba(255,255,255,.14)'; b.onmouseleave=()=>b.style.background='rgba(255,255,255,.08)'; b.onmousedown=()=>b.style.transform='scale(.97)'; b.onmouseup=()=>b.style.transform=''; return b; }
    const disp = document.createElement('div');
    disp.style.cssText = 'grid-column: span 6; text-align:center;font-weight:700;letter-spacing:.4px;margin-bottom:2px;';
    function setDisp(){ disp.textContent = 'Speed: x' + (speed).toFixed(2); }
    setDisp();
    const bDec = mkBtn('−');
    const bInc = mkBtn('+');
    const bHalf = mkBtn('1/2');
    const bDbl = mkBtn('×2');
    const bReset = mkBtn('Reset');
    const bSave = mkBtn('Save');
    [bDec,bInc,bHalf,bDbl,bReset,bSave].forEach(x=>{ x.style.userSelect='none'; });
    row.appendChild(disp);
    row.appendChild(bDec);
    row.appendChild(bInc);
    row.appendChild(bHalf);
    row.appendChild(bDbl);
    row.appendChild(bReset);
    row.appendChild(bSave);
    ctrl.appendChild(bClose);
    ctrl.appendChild(row);
    (document.body || document.documentElement).appendChild(ctrl);

    function placeCtrl(){
      const rect = ui.getBoundingClientRect();
      let x = rect.left + UI_SIZE + 10;
      let y = rect.top;
      ctrl.style.left = Math.min(window.innerWidth - 210, x) + 'px';
      ctrl.style.top = Math.min(window.innerHeight - 120, Math.max(10,y)) + 'px';
    }
    function showCtrl(){ placeCtrl(); setDisp(); ctrl.style.display='block'; }
    function hideCtrl(){ ctrl.style.display='none'; }
    function toggleCtrl(){ if (ctrl.style.display==='none' || !ctrl.style.display) showCtrl(); else hideCtrl(); }
    function setSpeed(v){ speed = Math.max(0.1, Math.min(64, v)); applySpeed(); started = speed !== SPEED_NORMAL; updateIcon(); pulse(); edgeIdle.reset(); setDisp(); }
    bDec.onclick = ()=> setSpeed(speed - 0.25);
    bInc.onclick = ()=> setSpeed(speed + 0.25);
    bHalf.onclick = ()=> setSpeed(speed / 2);
    bDbl.onclick = ()=> setSpeed(speed * 2);
    bReset.onclick = ()=> setSpeed(SPEED_NORMAL);
    bSave.onclick = ()=> { try { localStorage.setItem(STORAGE_KEY_DEFAULT, String(speed)); defaultFast = speed; } catch {} toast('Saved x'+speed.toFixed(2)); };
    ...
    let holdTO = null, holdTrig = false;
    ui.addEventListener('mousedown', e => {
      ...
      holdTrig = false;
      clearTimeout(holdTO);
      holdTO = setTimeout(() => { holdTrig = true; try { localStorage.setItem(STORAGE_KEY_DEFAULT, String(speed)); defaultFast = speed; } catch {} toast('Default x'+speed.toFixed(2)); }, 700);
    });
    ...
    ui.addEventListener('touchstart', e => {
      ...
      holdTrig = false;
      clearTimeout(holdTO);
      holdTO = setTimeout(() => { holdTrig = true; try { localStorage.setItem(STORAGE_KEY_DEFAULT, String(speed)); defaultFast = speed; } catch {} toast('Default x'+speed.toFixed(2)); }, 800);
    }, { passive: false });
    ...
    function toggle() {
      started = !started;
      speed = started ? defaultFast : SPEED_NORMAL;
      updateIcon(); applySpeed(); pulse(); edgeIdle.reset(); setDisp();
    }
    let clickTO = null;
    ui.addEventListener('click', e => {
      if (!dragging && !holdTrig) {
        clearTimeout(clickTO);
        clickTO = setTimeout(() => { toggle(); }, 200);
      }
    });
    ui.addEventListener('dblclick', e => { if (!dragging) { e.preventDefault(); clearTimeout(clickTO); toggleCtrl(); }});
    ...
    function toast(t){
      ...
      n.style.cssText = `position:fixed;z-index:${UI_ZINDEX};padding:8px 10px;border-radius:10px;`+
        `background:rgba(20,28,40,.75);color:#e8f6ff;border:1px solid rgba(120,180,210,.25);`+
        `box-shadow:0 8px 22px rgba(0,0,0,.25);font-size:12px;left:${(parseFloat(ui.style.left)||0)+UI_SIZE+10}px;top:${parseFloat(ui.style.top)||0}px;`;
      n.textContent = t; (document.body||document.documentElement).appendChild(n);
      setTimeout(()=>{ n.remove(); }, 900);
    }

    function isEdit(el){ return el && (el.tagName==='INPUT' || el.tagName==='TEXTAREA' || el.isContentEditable); }
    window.addEventListener('keydown', e => {
      if (isEdit(document.activeElement)) return;
      if (e.key === '0' && (e.ctrlKey||e.metaKey)) { e.preventDefault(); speed = SPEED_NORMAL; applySpeed(); started = false; updateIcon(); pulse(); }
      if ((e.key === '=' || e.key === '+') && (e.ctrlKey||e.metaKey)) { e.preventDefault(); speed = Math.min(64, speed + 0.25); applySpeed(); started = speed!==SPEED_NORMAL; updateIcon(); }
      if ((e.key === '-' || e.key === '_') && (e.ctrlKey||e.metaKey)) { e.preventDefault(); speed = Math.max(0.1, speed - 0.25); applySpeed(); started = speed!==SPEED_NORMAL; updateIcon(); }
      if ((e.key === '=' || e.key === '+') && e.altKey) { e.preventDefault(); speed = Math.min(64, speed * 2); applySpeed(); started = speed!==SPEED_NORMAL; updateIcon(); }
      if ((e.key === '-' || e.key === '_') && e.altKey) { e.preventDefault(); speed = Math.max(0.1, speed / 2); applySpeed(); started = speed!==SPEED_NORMAL; updateIcon(); }
      if ((e.key === 'd' || e.key === 'D') && (e.ctrlKey||e.metaKey)) { e.preventDefault(); try { localStorage.setItem(STORAGE_KEY_DEFAULT, String(speed)); defaultFast = speed; } catch {} toast('Default x'+speed.toFixed(2)); }
      if ((e.key === 's' || e.key === 'S') && (e.ctrlKey||e.metaKey)) { e.preventDefault(); toggleCtrl(); }
    });
