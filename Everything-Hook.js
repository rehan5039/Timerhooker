        ...
        return {
            ...
            getNowBetweenADay: function (date, isCeil, type) {
                ...
                if (!(date instanceof Date)) {
                    throw new TypeError('The argument type must be Date')
                }
                var time = date.getTime();
                var now = new Date();
                var nowTime = now.getTime();
                if (nowTime - time < 0) {
                    logger.warn('The time to compute must be before the current time');
                }
                var result = 0;
                switch (type) {
                    default:
        ...
        return {
            ...
            readLinkProperty: function (obj, linkProperty, cb) {
                ...
                if (typeof linkProperty === 'string') {
                    ...
                    if (normalResult === null) {
                        console.warn(obj, "property ['" + linkProperty + "'] value not found");
                    }
                    if (callback) {
            ...
            createLinkProperty: function (obj, linkProperty, value) {
                ...
                if (typeof linkProperty === 'string') {
                    ...
                    // 不判断为空的值
                    if (linkProperty === '') {
                        throw new TypeError('Object property name cannot be empty')
                    }
                    if (linkProperty.indexOf(',') !== -1) {
                        var propertyNameArr = linkProperty.split(',');
